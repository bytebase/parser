...
type TSqlParserListener interface {
	...
	// EnterCreate_spatial_index is called when entering the create_spatial_index production.
	EnterCreate_spatial_index(c *Create_spatial_indexContext)

	// EnterSpatial_tessellation_scheme is called when entering the spatial_tessellation_scheme production.
	EnterSpatial_tessellation_scheme(c *Spatial_tessellation_schemeContext)

	// EnterSpatial_index_options is called when entering the spatial_index_options production.
	EnterSpatial_index_options(c *Spatial_index_optionsContext)

	// EnterSpatial_index_option is called when entering the spatial_index_option production.
	EnterSpatial_index_option(c *Spatial_index_optionContext)

	// EnterSigned_decimal is called when entering the signed_decimal production.
	EnterSigned_decimal(c *Signed_decimalContext)

	// EnterSpatial_grid_level is called when entering the spatial_grid_level production.
	EnterSpatial_grid_level(c *Spatial_grid_levelContext)

	// EnterSpatial_grid_density is called when entering the spatial_grid_density production.
	EnterSpatial_grid_density(c *Spatial_grid_densityContext)
	...
	// EnterColumn_name_with_order is called when entering the column_name_with_order production.
	EnterColumn_name_with_order(c *Column_name_with_orderContext)
	...
	// ExitCreate_spatial_index is called when exiting the create_spatial_index production.
	ExitCreate_spatial_index(c *Create_spatial_indexContext)

	// ExitSpatial_tessellation_scheme is called when exiting the spatial_tessellation_scheme production.
	ExitSpatial_tessellation_scheme(c *Spatial_tessellation_schemeContext)

	// ExitSpatial_index_options is called when exiting the spatial_index_options production.
	ExitSpatial_index_options(c *Spatial_index_optionsContext)

	// ExitSpatial_index_option is called when exiting the spatial_index_option production.
	ExitSpatial_index_option(c *Spatial_index_optionContext)

	// ExitSigned_decimal is called when exiting the signed_decimal production.
	ExitSigned_decimal(c *Signed_decimalContext)

	// ExitSpatial_grid_level is called when exiting the spatial_grid_level production.
	ExitSpatial_grid_level(c *Spatial_grid_levelContext)

	// ExitSpatial_grid_density is called when exiting the spatial_grid_density production.
	ExitSpatial_grid_density(c *Spatial_grid_densityContext)
	...
	// ExitColumn_name_with_order is called when exiting the column_name_with_order production.
	ExitColumn_name_with_order(c *Column_name_with_orderContext)
