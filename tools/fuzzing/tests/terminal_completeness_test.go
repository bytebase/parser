...
func TestPostgreSQLTerminalCompleteness(t *testing.T) {
	...
	if nonTerminalNodes == 0 {
		...
		analyzeNonTerminalRules(parsedGrammar, depGraph, nonTerminalRules[:min(5, len(nonTerminalRules))], t)
	}
}
...
func analyzeNonTerminalRules(parsedGrammar *grammar.ParsedGrammar, depGraph *grammar.DependencyGraph, ruleNames []string, t *testing.T) {
